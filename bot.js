    ...
    conn.on('credentials-updated', async () => {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(
                    conn.user.jid,
                    '```SEEMS YOU WANT TO SWITCH THE WORK TYPE TO PRIVATE!```\n\n', MessageType.text
                );
                await heroku.patch(baseURI + '/config-vars', {
                    body: {
                        ['WORK_TYPE']: 'private'
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(
                    conn.user.jid,
                    '```SEEMS YOU WANT TO SWITCH THE WORK TYPE TO PUBLIC!```\n\n', MessageType.text
                );
                await heroku.patch(baseURI + '/config-vars', {
                    body: {
                        ['WORK_TYPE']: 'public'
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                return await conn.sendMessage(
                    conn.user.jid,
                    '```INVALID PARAMETERS`` \n```PLEASE USE``` < *.setvar WORK_TYPE:public* > ```OR``` < *.setvar WORK_TYPE:private* >\n\n', MessageType.text
                );
            }
        }
    })
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((OWN.ff == "94703228470,0" && msg.key.fromMe === false && command.fromMe === true &&
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [Whats] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [Whats] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
